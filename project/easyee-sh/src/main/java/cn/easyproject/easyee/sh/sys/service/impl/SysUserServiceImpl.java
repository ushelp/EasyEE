...
public class SysUserServiceImpl extends BaseService implements SysUserService {
	...
	public SysUser login(SysUser sysUser) {
		...
		SysUser user = super.uniqueResult(commonDAO.find(hql,
				sysUser.getName(), sysUser.getPassword()));
		if(user!=null){
	...
	public void changePwd(int id, String pwd) {
			commonDAO.updateByJpql(
					"update SysUser set password=? where userId=?", pwd, id);
	}
	...
	public boolean existsName(String name, Integer userId) {
		//修改用户时，检测用户名是否存在
			return commonDAO.findCount("select count(*) from SysUser where lower(name)=? and userId!=?", name.toLowerCase(),userId)>0;
	}
