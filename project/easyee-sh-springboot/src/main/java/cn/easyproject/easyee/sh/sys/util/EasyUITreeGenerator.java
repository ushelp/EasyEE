...
public class EasyUITreeGenerator {
	...
	static List<EasyUITreeEntity> getUserEasyUITreeMenu(Set<SysMenuPermission> menus) {
		...
		try {
			List<SysMenuPermission> rootMenus = (List<SysMenuPermission>) Ognl
					.getValue("#menus.{?#this.sysMenuPermission==null}", context, context.getRoot());

			sortMenuPermission(rootMenus); // 按SortNum排序菜单

			for (SysMenuPermission root : rootMenus) {
				EasyUITreeEntity easyUITreeRoot = new EasyUITreeEntity(root.getMenuPermissionId() + "", root.getName(),
		...
		try {
			...
			for (SysMenuPermission root : rootMenus) {
				EasyUITreeEntity easyUITreeRoot = new EasyUITreeEntity("menu_" + root.getMenuPermissionId() + "",
						root.getName(), root.getAction(), "open", root.getIcon());
				easyUITreeRoot.setRemark(root.getRemark());
				easyUITreeRoot.setType(EasyUITreeEntity.MENU_PERMISSION);
	 ...
	 *            EasyUITree Root对象
	 */
	private static void recursionAllInitEasyUITreeUsePrefixId(SysMenuPermission menuPermission, EasyUITreeEntity root) {
		...
		if (childMenus.size() > 0) {
			...
			for (SysMenuPermission child : childMenus) {
				EasyUITreeEntity easyUITreeChild = new EasyUITreeEntity("menu_" + child.getMenuPermissionId() + "",
						child.getName(), child.getAction(), "open", child.getIcon());
	 ...
	 * 初始化菜单下的权限树
	 * 
	 * @param easyUITreeEntity
	 *            EasyUITree节点实体
	 * @param sysOperationPermissions
	 *            操作权限集合
	 */
	private static void initOperationPermissionTreeUsePrefixId(EasyUITreeEntity easyUITreeEntity,
		...
		if (sysOperationPermissions != null) {
			for (SysOperationPermission sysOperationPermission : sysOperationPermissions) {
				EasyUITreeEntity treeEntity = new EasyUITreeEntity(
						"operation_" + sysOperationPermission.getOperationPermissionId() + "",
						sysOperationPermission.getName());
	 ...
	 *            操作权限集合
	 */
	private static void initOperationPermissionTree(EasyUITreeEntity easyUITreeEntity,
			Set<SysOperationPermission> sysOperationPermissions) {
		if (sysOperationPermissions != null) {
			for (SysOperationPermission sysOperationPermission : sysOperationPermissions) {
				EasyUITreeEntity treeEntity = new EasyUITreeEntity(
						sysOperationPermission.getOperationPermissionId() + "", sysOperationPermission.getName());
				treeEntity.setRemark(sysOperationPermission.getRemark());
				treeEntity.setUrl(sysOperationPermission.getAction());
				treeEntity.setType(EasyUITreeEntity.OPERATION_PERMISSION);
				easyUITreeEntity.getChildren().add(treeEntity);
	 ...
	 *            EasyUITree Root对象
	 */
	private static void recursionAllInitEasyUITree(SysMenuPermission menuPermission, EasyUITreeEntity root) {
		...
		if (childMenus.size() > 0) {
			...
			for (SysMenuPermission child : childMenus) {
				EasyUITreeEntity easyUITreeChild = new EasyUITreeEntity(child.getMenuPermissionId() + "",
						child.getName(), child.getAction(), "open", child.getIcon());
				easyUITreeChild.setRemark(child.getRemark());
				easyUITreeChild.setType(EasyUITreeEntity.MENU_PERMISSION);
				root.getChildren().add(easyUITreeChild);
	 ...
	 *            EasyUITree Root对象
	 */
	private static void recursionInitEasyUITree(SysMenuPermission menuPermission, EasyUITreeEntity root) {
	...
	private static void recursionInitEasyUITree(EasyUITreeEntity root) {
		...
		try {
			List<SysMenuPermission> childMenus = (List<SysMenuPermission>) Ognl
					.getValue("#menus.{?#this.sysMenuPermission!=null&&#this.sysMenuPermission.menuPermissionId=="
