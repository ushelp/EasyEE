...
<div id="sysRoleAddDialog" class="easyui-dialog"
	...
	<form id="sysRoleAddForm" method="post">
			<input type="hidden" name="sysRole.roleId"/>
			 <table cellpadding="5">
	    		<tr>
	    			<td>角色名:</td>
	    			<td><input class="easyui-textbox easyui-validatebox" name="sysRole.name"
				 required="true"
	    		...
	    		<tr>
	    			<td>状态:</td>
	    			<td>
						<input type="radio" name="sysRole.status" value="0" id="on" checked="checked"  />
						<label for="on">启用</label> 
						<input type="radio" name="sysRole.status" value="1" id="off" />
						<label for="off">禁用</label>
					</td>
	    		</tr>
	    		<tr>
	    			<td>菜单备注:</td>
	    			<td><input class="easyui-textbox" multiline="true" style="height:40px;width: 200px;" name="sysRole.remark"></td>
	    		</tr>
	    		<tr>
	    			<td>权限：</td>
	    			<td>
...
<div id="sysRoleAdd-dlg-buttons">
	<a class="easyui-linkbutton" id="sysRoleAddSaveBtn" iconCls="icon-ok"
		onclick="sysRole.add()">添加</a> <a class="easyui-linkbutton"
		iconCls="icon-cancel"
